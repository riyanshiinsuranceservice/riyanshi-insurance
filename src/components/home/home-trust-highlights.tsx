...
function HomeTrustHighlights({ items }: HomeTrustHighlightsProps) {
  return (
    <section
      id="about"
      className="bg-[var(--color-surface-container-low)] py-14 md:py-16"
    >
      <div className="mx-auto grid w-full max-w-7xl gap-8 px-6 md:grid-cols-3">
        {items.map((item, index) => {
          ...
          return (
            <div
              key={item.title}
              className="surface-card elevation-ambient flex items-center gap-6 rounded-2xl border border-[color:color-mix(in_srgb,var(--color-outline-variant)_12%,transparent)] p-8 shadow-sm"
            >
              <div
              ...
              <div>
                <h3 className="font-[var(--font-display)] text-xl font-bold text-[var(--color-primary)]">
                  {item.title}
                </h3>
